...
# Эта конструкция один из путей ее реализации, она позволяет изолировать (не импортировать)  методы, классы если они есть в модуле.
#
...
# В этом случае, то что написано под капотом внизу, под if __name__ == "__main__":  исполняться(передаваться в код) name_other_file.py не будет.
